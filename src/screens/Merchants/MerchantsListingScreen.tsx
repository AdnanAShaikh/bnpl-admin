...
const MerchantsListingScreen = () => {
 ...
 // Flatten + filter by tab
  ...
  const dataSource = activeTab === "pending"
...
const ROW_ACTIONS: RowAction<FlatMerchant>[] = [
  canView && {
    ...
    onClick: (merchant:FlatMerchant) => navigate(`/admin/merchant/${merchant.id}`),
  },
  canEdit && {
    label: "Edit Merchant",
    icon: (
    ...
    onClick: (merchant: FlatMerchant) => navigate(`/admin/merchant/edit/${merchant.id}`),
  },
  ...
  return (
    <Sidebar>
      ...
      <DataTable<FlatMerchant>
        ...
        statusOptions={["DRAFT", "PENDING_APPROVAL", "APPROVED", "REJECTED", "SUSPENDED", "All Status"]}
