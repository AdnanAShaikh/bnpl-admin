        ...
        <div className="flex justify-center mt-4 mb-6">
          {canResend ? (
            ...
            <p className="text-sm text-gray-400">
              Resend{" "}
              <span className="font-semibold text-[#1a3a6a]">
...
const LoginScreen = () => {
  ...
  const handleLogin = async () => {
    ...
    const result = await dispatch(
      loginAdmin({ email, password, role: "ADMIN" }),
    );
  ...
  const handleVerifyOtpLogin = async (code: string) => {
    const result = await dispatch(
      verifyOtpForLogin({ email, code, role: "ADMIN" }),
    );
        ...
        <div className="w-full max-w-[440px] bg-white rounded-2xl shadow-2xl shadow-black/30 p-9 relative z-10">
          ...
          <div className="flex items-center justify-between mb-7">
            ...
            <button
              ...
              type="button"
              className="cursor-pointer text-sm font-semibold text-primary hover:text-secondary transition-colors"
            >
              Forgot password?
            </button>
          </div>
          ...
          <button
            ...
            onClick={handleLogin}
            disabled={isLoading}
            className="w-full py-3.5 rounded-xl bg-primary text-white font-bold text-base tracking-wide transition-all duration-200 hover:bg-[#22335a] active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed shadow-lg shadow-primary/30"
          >
            {isLoading ? (
              <span className="flex items-center justify-center gap-2">
                <svg
