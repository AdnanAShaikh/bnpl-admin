...
const CreateNewBuyerScreen = () => {
  ...
  const handleSubmit = async () => {
    ...
    const result = await dispatch(createNewBuyer({
      name:     form.name,
      email:    form.email,
      password: form.password,
    }));

    if (createNewBuyer.fulfilled.match(result)) {
      const buyerId = result.payload.user.buyer.id;
      toast.success("Buyer Created!");
      await dispatch(fetchAllBuyers()); // ← refetch so edit screen finds it in state
      navigate(`/admin/buyer/edit/${buyerId}`); // go straight to edit screen
    } else {
      toast.error(result.payload as string || "Failed to create buyer");
    }
  };
  ...
  return (
    <Sidebar>
      <div className="flex items-center justify-between mb-5">
        <h1 className="text-xl font-bold text-[#1a2a4a]">Create New Buyer</h1>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        <div className="p-7 flex flex-col gap-6">
          ...
          <p className="text-sm text-gray-500 leading-relaxed">
            Enter the buyer's basic credentials. You'll be taken to their profile to fill in company details and upload documents.
          </p>
          ...
          <div className="flex justify-end pt-2 border-t border-gray-100">
            ...
            >
              {actionLoading ? "Creating..." : "Create Buyer"}
            </button>
          </div>
...
export default CreateNewBuyerScreen;